...
# Fixture for --unit example: flaky_spec.rb has a before(:all) hook that counts its
# runs in HOPPER_FIXTURE_STATE_DIR, one example that fails on its first attempt and
# passes from the second (attempts counted the same way), a steady example and a
# nested one; hard_spec.rb has one example that always fails and one that passes.
